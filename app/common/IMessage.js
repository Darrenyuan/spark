...
export default class IMessage {
  ...
  ping = () => {
    return this._createMessage({}, EVENTS.PING).then(data => {
  ...
  createSingleText = ({ text, toId, ...others }) => {
    return this._createText(text, others).then(data => {
  ...
  createSingleImage = ({ image, toId, ...others }) => {
    return this._createImage(image, others).then(data => {
      data = this.setSingleData(data, toId);
      return this._createMessage(data, EVENTS.SEND_SINGLE_MSG);
    });
  };
  ...
  createSingleVoice = ({ voice, toId, ...others }) => {
    return this._createVoice(voice, others).then(data => {
      data = this.setSingleData(data, toId);
      return this._createMessage(data, EVENTS.SEND_SINGLE_MSG);
    });
  };
  ...
  createSingleVideo = ({ video, toId, ...others }) => {
    return this._createVideo(video, others).then(data => {
      data = this.setSingleData(data, toId);
      return this._createMessage(data, EVENTS.SEND_SINGLE_MSG);
    });
  };
  ...
  createSingleLocation = ({ location, toId, ...others }) => {
    return this._createLocation(location, others).then(data => {
      data = this.setSingleData(data, toId);
      return this._createMessage(data, EVENTS.SEND_SINGLE_MSG);
    });
  };
  ...
  createSingleNotification = (notification, toId) => {
    return this._createNotification(notification).then(data => {
      data = this.setSingleData(data, toId);
      return this._createMessage(data, EVENTS.SEND_SINGLE_MSG);
    });
  };
  ...
  sendRequestAddFriend = (toUserId, text) => {
    ...
    //发送好友请求验证
    return this._createMessage(data, EVENTS.REQUEST_ADD_FRIEND).then(data => this.send(data));
  };

  sendSystemNotice = ({ toUserId, noticeId, noticeType }) => {
    return this._createMessage({ toUserId, noticeId, noticeType }, EVENTS.SEND_SYSTEM_NOTICE).then(
      data => this.send(data),
    );
  };
}
