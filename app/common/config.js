...
let config = {
  ...
  reportItems() {
    return ['无意义的内容（灌水）', '恶意攻击谩骂', '营销广告', '淫秽色情', '政治反动', '其他'];
  },
  generateGroup(data = []) {
    let newMap = {};
  ...
  saveConversationList(data = {}) {
    ...
    return config.getChatList().then(chatList => {
      for (let i = 0; i < data.length; i++) {
        ...
        if (newData.chatType === 2 && newData.gid) {
          //1为单聊，2群聊
  ...
  saveRequestAddFriendList(data = []) {
    ...
    // console.warn(data);
    //保存请求好友列表，返回未读数
    return config.getRequestAddFriendList().then(list => {
      let obj = {}; //存储未读数
      let unreadMsg = 0;
      const arr = [];
      data.forEach((v, i) => {
  ...
  resetRequestAddFriends() {
    return config.getRequestAddFriendList().then(data => {
      data.forEach((v, i) => {
        data[i].unreadMsg = 0;
      });
      return data;
  ...
  saveDynamicMsg(data = []) {
    ...
    return config.getDynamicMsg().then(list => {
