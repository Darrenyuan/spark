...
export default class AddDynamic extends React.Component {
  ...
  rightOnPress = () => {
    ...
    if (data.selectedImages.length > 0) {
      ...
      for (let item of data.selectedImages) {
        request
          .upload(config.api.baseURI + config.api.uploadImage, {
            uri: item.path,
            ext: 'jpg',
          })
          .then(res => {
      ...
      request
        ...
        .then(thumb => {
          ...
          return request
            .upload(config.api.baseURI + config.api.uploadVideo, {
              uri: data.video.path,
              ext: 'mp4',
            })
            .then(res => {
  ...
  submit = (data = {}) => {
    request
      .post(config.api.baseURI + config.api.addDynamic, data)
      .then(res => {
    ...
    if (source.path.lastIndexOf('.MOV') > -1) {
      ...
      RNThumbnail.get(source.path)
        .then(result => {
          ...
          ProcessingManager.compress(source.path, options)
            .then(data => {
              toast.modalLoadingHide();
              source.path = data;
      ...
      source.path.lastIndexOf('.jpg') > -1 ||
      source.path.lastIndexOf('.png') > -1 ||
      source.path.lastIndexOf('.gif') > -1
    ) {
      toast.modalLoadingHide();
  ...
  _compress = source => {
    ...
    const OutputVideoPath =
      '/storage/emulated/0/DCIM/Camera/temp_' +
      InputVideoPath.substring(InputVideoPath.lastIndexOf('/') + 1);
    const cmd =
      '-y -i ' +
      InputVideoPath +
      ' -s 960x540 -strict -2 -vcodec libx264 -preset faster ' +
      '-crf 24 -acodec aac -ar 44100 -ac 2 -b:a 96k -vf transpose=1 -aspect 16:9 ' +
      OutputVideoPath;
  ...
  renderAddButton = () => {
    ...
    return (
      <TouchableOpacity style={styles.addButton} activeOpacity={0.5} onPress={this.showAction}>
        <Icon name={'ios-add'} type={'ionicon'} size={50} color={'#ccc'} />
      </TouchableOpacity>
    );
  };
  ...
  renderCountText = () => {
    ...
    return (
      ...
      >
        <Text style={[styles.sendText, { color: text.length >= MAX_LENGTH ? 'red' : '#3CB371' }]}>
  ...
  addSubject = text => {
    ...
    request
      .post(config.api.baseURI + config.api.addSubject, {
        subjectName: text,
      })
      .then(res => {
        if (res.code === 0) {
